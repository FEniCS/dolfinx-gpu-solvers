...
t_end = 1.0
num_time_steps = 1000   # CFL is tighter for DG1; increase if blow-up
...
# ── KSP for M·k = R (block-diagonal DG mass matrix → LU is cheap) ────────────
...
# Temporary Function to hold M^{-1} R result
k_fn = fem.Function(V)
...
def compute_Minv_R(u_input: fem.Function) -> np.ndarray:
    """Evaluate M^{-1} R(u_input).  Returns a *copy* of the local array."""
    ...
    ksp.solve(b, k_fn.x.petsc_vec)
    k_fn.x.scatter_forward()
    return k_fn.x.array.copy()
