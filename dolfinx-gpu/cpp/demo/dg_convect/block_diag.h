        ...
        std::reference_wrapper<const dolfinx::fem::DirichletBC<T, U>>>& bcs)
{
  ...
  if constexpr (std::is_same_v<double, T>)
  {
    BLAS_CHECK(gpublasDmatinvBatched(handle, n, ptrA_device.data().get(), n,
                                     ptrAinv_device.data().get(), n,
                                     info.data().get(), ncells));
  }
  else if constexpr (std::is_same_v<float, T>)
  {
    BLAS_CHECK(gpublasSmatinvBatched(handle, n, ptrA_device.data().get(), n,
                                     ptrAinv_device.data().get(), n,
                                     info.data().get(), ncells));
  }
  else
...
/// @param u Solution vector
template <dolfinx::scalar T>
void solve_block_diag_system(const thrust::device_vector<T>& Ainv,
                             ...
                             thrust::device_vector<T>& u)
{
  ...
  {
    ptrAinv[i] = Ainv.data().get() + n * n * i;
    ptrb[i] = b.data().get() + n * i;
    ptru[i] = u.data().get() + n * i;
  }
  ...
  if constexpr (std::is_same_v<double, T>)
  {
    BLAS_CHECK(gpublasDgemvBatched(handle, GPUBLAS_OP_T, n, n, &alpha,
                                   ptrAinv_device.data().get(), n,
                                   ptrb_device.data().get(), 1, &beta,
                                   ptru_device.data().get(), 1, ncells));
  }
  ...
  {
    BLAS_CHECK(gpublasSgemvBatched(handle, GPUBLAS_OP_T, n, n, &alpha,
                                   ptrAinv_device.data().get(), n,
                                   ptrb_device.data().get(), 1, &beta,
                                   ptru_device.data().get(), 1, ncells));
  }
